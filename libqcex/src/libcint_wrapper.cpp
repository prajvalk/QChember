...
void _destroy_libcint_environment(int* atm, int* bas, double* env) {
...
void _cint1e_wrapper (IntegralEngineHandle* handle, CINTGenericIntegral integral_function, CINTGenericOptimizer opt_function, Matrix<double>** output) {
    double* buffer = new double[handle->nbf_max * handle->nbf_max];
    int*    shells = new int   [2];
    int*    dims   = new int   [3] {1, 1, 1};
    (*output) = new Matrix<double>(handle->nbf_tot, handle->nbf_tot);
    CINTOpt* opt = nullptr;
    opt_function (&opt, handle->atm, handle->natm, handle->bas, handle->nbas, handle->env);
    for (int i = 0; i < handle->nbas; i++) {
        for (int j = 0; j <= i; j++) {
            ...
            memset(buffer, 0, sizeof(double) * dims[0] * dims[1]);
            int ncomp = integral_function (buffer, dims, shells, handle->atm, handle->natm, handle->bas, handle->nbas, handle->env, opt, handle->cache);
            if (ncomp != 1) HANDLE_ERROR ("libcint 1e calculation failed.", 101);
            for (int ii = 0; ii < dims[0]; ii++) {
                for (int jj = 0; jj < dims[1]; jj++) {
                    double val = buffer[ii * dims[1] + jj];
                    (*output)->set(ioff + ii, joff + jj, val);
                    (*output)->set(joff + jj, ioff + ii, val);
                }
            }
        }
    }
...
namespace newscf::qcex {
    ...
    void  destroy_handle   (IntegralEngineHandle* handle) {
        _destroy_libcint_environment (handle->atm, handle->bas, handle->env);
        free(handle->shell_to_index);
        free(handle->cache);
    }

    void  calculate_overlap_matrix(IntegralEngineHandle* handle, Matrix<double>** output) {
        _cint1e_wrapper (handle, int1e_ovlp_sph, int1e_ovlp_optimizer, output);
    }
