...
int main() {
    TestHandle h;
    h.testsuite = "qcex_basis";

    init_tests();

    add_test("def2-SVP_load", h);

    double* bswork;

    newscf::qcex::load_basis("def2-SVP.bas", &bswork);
    ...
    newscf::qcex::destroy_basis(bswork);

    complete_test(h);

    add_test("def2-TZVPP_load", h);

    load_basis("def2-TZVPP.bas", &bswork);
